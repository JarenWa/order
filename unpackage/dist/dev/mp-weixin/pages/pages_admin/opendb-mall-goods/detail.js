    ...
    async loadAllCategories() {
      try {
        ...
        if (res && Array.isArray(res.data)) {
          ...
          common_vendor.index.__f__("log", "at pages/pages_admin/opendb-mall-goods/detail.vue:154", "分类映射加载完成", this.categoryMap);
        } else {
          common_vendor.index.__f__("warn", "at pages/pages_admin/opendb-mall-goods/detail.vue:156", "分类加载返回格式异常", res);
        }
      } catch (err) {
        common_vendor.index.__f__("error", "at pages/pages_admin/opendb-mall-goods/detail.vue:159", "加载分类失败", err);
      }
    },
    // 根据分类ID获取名称
      ...
      common_vendor.index.showModal({
        ...
        success: (res) => {
          if (res.confirm) {
            ...
            db.collection("opendb-mall-goods").doc(this._id).remove().then(() => {
...
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  return common_vendor.e({
    ...
    x: common_vendor.t($data.data.is_on_sale ? "是" : "否"),
    y: common_vendor.n($data.data.is_on_sale ? "active" : "inactive"),
    z: common_vendor.t($data.data.is_pre ? "否" : "是"),
    A: common_vendor.n($data.data.is_pre ? "inactive" : "active"),
    B: common_vendor.p({
      title: "状态",
      ["is-shadow"]: true,
      margin: "10px 0"
    }),
    C: common_vendor.t($data.data.tag ? $data.data.tag.join("、") : "无"),
    D: common_vendor.t($data.data.total_sell_count || 0),
    E: common_vendor.t($options.formatDate($data.data.add_date)),
    F: common_vendor.t($options.formatDate($data.data.last_modify_date)),
    G: common_vendor.t($data.data.operater || "未知"),
    H: common_vendor.p({
      title: "其他信息",
      ["is-shadow"]: true,
      margin: "10px 0"
    ...
    J: common_vendor.o((...args) => $options.handleDelete && $options.handleDelete(...args))
  });
}
const MiniProgramPage = /* @__PURE__ */ common_vendor._export_sfc(_sfc_main, [["render", _sfc_render], ["__scopeId", "data-v-7934c8ff"]]);
wx.createPage(MiniProgramPage);
//# sourceMappingURL=../../../../.sourcemap/mp-weixin/pages/pages_admin/opendb-mall-goods/detail.js.map
